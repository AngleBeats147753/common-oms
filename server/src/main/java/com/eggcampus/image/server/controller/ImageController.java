...
import com.eggcampus.image.server.pojo.qo.*;
...
public class ImageController {
    ...
    @PutMapping("/image/useList")
    public ReturnResult useImageList(@Validated @RequestBody BatchUsageQO qo) {
        imageService.useImageList(qo.getUsageQoList());
        return ReturnResult.getSuccessReturn("使用图像列表成功");
    }

    @DeleteMapping("/image/removeList")
    public ReturnResult deleteImageList(@Validated @RequestBody BatchDeleteQO deleteQO) {
        imageService.deleteImageList(deleteQO.getDeleteQoList());
        return ReturnResult.getSuccessReturn("删除图像列表成功");
    }
}
